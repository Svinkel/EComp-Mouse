...
void ServiceRequests(void)
{
    ...
    if(USBGenRead((byte*)&dataPacket,sizeof(dataPacket)))
    {
        ...
        {
            case READ_VERSION:
                input = ReadWithSPIHalfDuplex(READ_VERSION);
				// dataPacket._byte[1] is len
                dataPacket._byte[2] = input; // Switch 2 status
                dataPacket._byte[3] = !PB5; // Switch 3 status

				counter=0x04;
                break;
			case MOUSE_RES:
                confirmation = WriteWithSPIHalfDuplex(MOUSE_RES, 0x01);
				// dataPacket._byte[1] is len
                dataPacket._byte[2] = confirmation; // Switch 2 status

				counter=0x03;
                break;
			case POSITIONX:
                x = ReadWithSPIHalfDuplex(0x03);
				// dataPacket._byte[1] is len
                dataPacket._byte[2] = x; // Switch 2 status
                dataPacket._byte[3] = !PB5; // Switch 3 status

				counter=0x04;
                break;
			case POSITIONY:
                y = ReadWithSPIHalfDuplex(0x04);
				// dataPacket._byte[1] is len
                dataPacket._byte[2] = y; // Switch 2 status
                dataPacket._byte[3] = !PB5; // Switch 3 status
            ...
            case POSITIONXY:
                x = ReadWithSPIHalfDuplex(0x03);
                y = ReadWithSPIHalfDuplex(0x04);
                dataPacket._byte[2] = x; 
                dataPacket._byte[3] = y; 

				counter=0x04;
				break;
			case MOTION:
                motion = ReadWithSPIHalfDuplex(0x02);
				// dataPacket._byte[1] is len
                dataPacket._byte[2] = motion; // Switch 2 status
                dataPacket._byte[3] = !PB5; // Switch 3 status
			...
			case PIXEL:
				for(i=2;i<47;i++){
					pixel = ReadWithSPIHalfDuplex(0x0b);
					while ((pixel & 0x80) != 0x80) {
						pixel = ReadWithSPIHalfDuplex(0x0b);
					}
	                dataPacket._byte[i] = pixel;
				}
			...
			case PIXELMOUSE:
				...
				break;
			case PIXEL_RST:
				pixel = WriteWithSPIHalfDuplex(PIXEL, 0x01);
				// dataPacket._byte[1] is len
                dataPacket._byte[2] = pixel; 
				dataPacket._byte[3] = pixel;
